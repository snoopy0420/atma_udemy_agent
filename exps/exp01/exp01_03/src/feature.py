...
class UdemyActivityDetailFeature(FeatureBase):
    """Udemyアクティビティの詳細特徴量を生成するクラス

    受講日数・月数・一気観比率・コース多様性・クイズ比率・
    平均受講時間・完了率トレンド・直近3ヶ月受講数等を集計する。
    """
    ...
    def _create_feature(self) -> pd.DataFrame:
        ...
        # 日付・月を抽出
        df_udemy['date'] = df_udemy['開始日'].dt.date
        df_udemy['year_month'] = df_udemy['開始日'].dt.to_period('M')

        # 直近3ヶ月判定に使う基準日（データ全体の最大開始日）
        max_date = df_udemy['開始日'].max()
        threshold_3m = max_date - pd.DateOffset(months=3)

        list_records = []
        for emp_id, df_emp in df_udemy.groupby('社員番号'):
            n = len(df_emp)

            # 受講日数・月数
            active_days = df_emp['date'].nunique()
            active_months = df_emp['year_month'].nunique()

            # 月あたりセッション数（日数ベース）
            sessions_per_month = active_days / active_months if active_months > 0 else 0

            # 完了率トレンド（直近3ヶ月平均 - それ以前の平均）
            df_recent = df_emp[df_emp['開始日'] > threshold_3m]
            df_past = df_emp[df_emp['開始日'] <= threshold_3m]
            recent_comp_mean = df_recent['推定完了率_'].mean() if len(df_recent) > 0 else np.nan
            past_comp_mean = df_past['推定完了率_'].mean() if len(df_past) > 0 else np.nan
            completion_trend = (
                recent_comp_mean - past_comp_mean
                if (not np.isnan(recent_comp_mean) and not np.isnan(past_comp_mean))
                else np.nan
            )

            # 一気観比率（同日に複数レクチャーがある日の割合）
            daily_counts = df_emp.groupby('date').size()
            binge_days = (daily_counts > 1).sum()
            binge_ratio = binge_days / active_days if active_days > 0 else 0

            # コースカテゴリ数（nunique）
            course_diversity = df_emp['コースカテゴリー'].nunique()

            # 平均レクチャー所要時間（分単位）
            duration_minutes = (df_emp['終了日'] - df_emp['開始日']).dt.total_seconds() / 60
            avg_lecture_duration = duration_minutes.mean()

            # クイズ比率（クイズ数 / 全レクチャー数）
            quiz_count = (df_emp['レクチャーもしくはクイズ'] == 'Quiz').sum()
            quiz_ratio = quiz_count / n if n > 0 else 0

            # クイズ平均スコア（クイズのみの最終結果）
            # カラム名は clean_feature_names により 最終結果_クイズの場合_
            score_col = '最終結果_クイズの場合_'
            if score_col in df_emp.columns:
                df_quiz = df_emp[df_emp['レクチャーもしくはクイズ'] == 'Quiz']
                quiz_score_mean = df_quiz[score_col].mean() if len(df_quiz) > 0 else np.nan
            else:
                quiz_score_mean = np.nan

            # マーク済み終了率
            marked_done_ratio = df_emp['マーク済み修了'].mean()

            # 直近3ヶ月のレクチャー数
            recent3m_count = len(df_recent)

            # コース完了数（コース単位で推定完了率_が100のコース数）
            df_course_comp = df_emp.groupby('コースID')['推定完了率_'].max()
            course_completion_count = (df_course_comp >= 100).sum()

            list_records.append({
                '社員番号': emp_id,
                'udemy_active_days': active_days,
                'udemy_active_months': active_months,
                'udemy_sessions_per_month': sessions_per_month,
                'udemy_completion_trend': completion_trend,
                'udemy_binge_ratio': binge_ratio,
                'udemy_course_diversity': course_diversity,
                'udemy_avg_lecture_duration': avg_lecture_duration,
                'udemy_quiz_ratio': quiz_ratio,
                'udemy_quiz_score_mean': quiz_score_mean,
                'udemy_marked_done_ratio': marked_done_ratio,
                'udemy_recent3m_count': recent3m_count,
                'udemy_course_completion_count': course_completion_count,
            })

        df_feature = pd.DataFrame(list_records)
        return df_feature


class OvertimeTimeSeriesFeature(FeatureBase):
    """残業データの時系列特徴量を生成するクラス

    月次残業時間の傾き・直近平均・変動係数・ゼロ連続月・前年比等を集計する。
    """
    ...
    def _create_feature(self) -> pd.DataFrame:
        ...
        # date でソート
        df_overtime = df_overtime.sort_values(['社員番号', 'date'])

        list_records = []
        for emp_id, df_emp in df_overtime.groupby('社員番号'):
            hours = df_emp['hours'].values
            n = len(hours)

            # 線形回帰の傾き（月次インデックスを説明変数）
            if n >= 2:
                slope, _, _, _, _ = stats.linregress(np.arange(n), hours)
            else:
                slope = np.nan

            # 直近3・6ヶ月平均
            recent3m_mean = hours[-3:].mean() if n >= 3 else hours.mean()
            recent6m_mean = hours[-6:].mean() if n >= 6 else hours.mean()

            # 最初の3ヶ月平均（入社直後の行動）
            first3m_mean = hours[:3].mean() if n >= 3 else hours.mean()

            # 変動係数（std / mean）
            mean_val = hours.mean()
            cv = hours.std() / mean_val if mean_val != 0 else np.nan

            # 残業ゼロでない月数
            nonzero_count = int((hours > 0).sum())
            ...
            # 最終月の残業時間
            last_value = float(hours[-1])

            # 前年比変化率（最終12ヶ月平均 / 最初12ヶ月平均 - 1）
            if n >= 24:
                yoy_change = hours[-12:].mean() / (hours[:12].mean() + 1e-9) - 1
            else:
                yoy_change = np.nan

            list_records.append({
                '社員番号': emp_id,
                'ot_ts_trend_slope': slope,
                'ot_ts_recent3m_mean': recent3m_mean,
                'ot_ts_recent6m_mean': recent6m_mean,
                'ot_ts_first3m_mean': first3m_mean,
                'ot_ts_cv': cv,
                'ot_ts_nonzero_count': nonzero_count,
                'ot_ts_max_consec_zero': max_consec_zero,
                'ot_ts_last_value': last_value,
                'ot_ts_yoy_change': yoy_change,
            })
